...
subroutine readSol(Rstep)  
  ...
  read(solf,*) (OmegaOld(j), j=1,3)
  read(solf,*) (OmegatangOld(j),j=1,3)
  do i = 1, 3
    read(solf,*) (RmatOld(i,j),j=1,3)
  end do
  do i = 1, 3
    read(solf,*) (RtangOld(i,j),j=1,3)
  end do
      
!A  read(solf,*) (vbn0(j),  j=1,3)
!A  read(solf,*) (dbn0(j),  j=1,3)
!A  read(solf,*) (wbn0(j),  j=1,3)
!A  read(solf,*) (Rn0(1,j), j=1,3)
!A  read(solf,*) (Rn0(2,j), j=1,3)
!A  read(solf,*) (Rn0(3,j), j=1,3)
!A  read(solf,*)  Mass_init
...
!======================================================================
subroutine generateIC(SH, NM, Center_Rot)
...
!A  thedd = theddOld
    ...
    thetd = 0.0d0
    thedd = 0.0d0
    theta = 0.0d0!thetaold + Delt*thetdold + Delt*Delt/2.0d0* &
            !           ((1.0d0-2.0d0*beti)*theddold+2.0d0*beti*thedd)

    call get_Rmat_Z(theta, thetd, thedd, Rt, Rdt, Rddt)
    ...
    do i = 1, NNODE
      if(NodeID(i) >= 32) then
        do j = 1, 3
          ugmold(i,j) = sum(Rdt(j,:)*(xg(i,1:3) - xrotOld(1:3)))
          acgmold(i,j) = sum(Rddt(j,:)*(xg(i,1:3) - xrotOld(1:3)))
        end do
      end if
    end do
      ...
      do i = 1, NM%FEM(1)%NNODE
        do j = 1, NSD
          NM%FEM(1)%ashOld(i,j) = sum(Rddt(j,:)*(NM%FEM(1)%B_NET(i,1:3) - xrotOld(1:3)))
          NM%FEM(1)%ushOld(i,j) = sum(Rdt(j,:)*(NM%FEM(1)%B_NET(i,1:3) - xrotOld(1:3)))
        end do        
      end do
  ...
  NM%FEM(2)%dshOld = 0.0d0
  NM%FEM(2)%dsh = NM%FEM(2)%dshOld

  NM%FEM(2)%ashOld = 0.0d0
  NM%FEM(2)%ushOld = 0.0d0

  NM%FEM(2)%ash = NM%FEM(2)%ashOld
  NM%FEM(2)%ush = NM%FEM(2)%ushOld

  NM%FEM(1)%dshOld = 0.0d0
  NM%FEM(1)%dsh = NM%FEM(1)%dshOld

  NM%FEM(1)%ash = NM%FEM(1)%ashOld
  NM%FEM(1)%ush = NM%FEM(1)%ushOld
...
!    SH%TSP%dshOld = 0.0d0
 !   SH%TSP%ushOld = 0.0d0
  !  SH%TSP%ashOld = 0.0d0
    ...
    do i = 1, SH%NRB%NNODE
      if(SH%NRB%B_NET(i,3) >= 3.0d0) then
        do j = 1, 3
          SH%NRB%ushOld(i,j) = sum(Rdt(j,:)*(SH%NRB%B_NET(i,1:3) - xrotOld(1:3)))
          SH%NRB%ashOld(i,j) = sum(Rddt(j,:)*(SH%NRB%B_NET(i,1:3) - xrotOld(1:3)))
        end do
      end if
    end do
    ...
    do i = 1, SH%FEM%NNODE
      if(SH%FEM%B_NET(i,3) >= 3.0d0) then
        do j = 1, 3
          SH%FEM%ushOld(i,j) = sum(Rdt(j,:)*(SH%FEM%B_NET(i,1:3) - xrotOld(1:3)))
          SH%FEM%ashOld(i,j) = sum(Rddt(j,:)*(SH%FEM%B_NET(i,1:3) - xrotOld(1:3)))
        end do
      end if
    end do
...
subroutine writeSol(istep)
  ...
  write(solf,*) (Omega(j), j=1,3)
  write(solf,*) (Omegatang(j),j=1,3)
  do i = 1, 3
    write(solf,*) (Rmat(i,j),j=1,3)
  end do
  do i = 1, 3
    write(solf,*) (Rtang(i,j),j=1,3)
  end do

!A  write(solf,*) (vbn1(j), j=1,3)
!A  write(solf,*) (dbn1(j), j=1,3)
!A  write(solf,*) (wbn1(j), j=1,3)
!A  write(solf,*) (Rn1(1,j),j=1,3)
!A  write(solf,*) (Rn1(2,j),j=1,3)
!A  write(solf,*) (Rn1(3,j),j=1,3)
!A  write(solf,*)  Mass_init
