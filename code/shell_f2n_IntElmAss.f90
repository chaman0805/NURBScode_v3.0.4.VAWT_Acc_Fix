...
!=======================================================================
subroutine f2n_IntElmAss(NRB, FEM, nsd, fg, &
                         RHSG_SH, mg_SH)
  ...
  do iel = 1, NRB%NEL
    ...
    if ( (NRB%U_KNOT(iel,ni) /= NRB%U_KNOT(iel,ni+1)) .and. &
         ...
         (NRB%PTYPE(iel) .ne. 2) .and. (NRB%PTYPE(iel) .ne. 0) ) then
      ...
      xKebe(5,:,:) = xMebe
      xKebe(9,:,:) = xMebe
!A      call FillSparseMat_3D_shell(nsd, nshl, lIEN, NRB%NNODE, &
!A                                  NRB%maxNSHL, icnt, col, row, &
!A                                  xKebe, LHSK_SH)
